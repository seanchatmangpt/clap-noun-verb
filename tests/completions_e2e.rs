...
fn test_completions_e2e_bash() {
    let mut cmd = Command::cargo_bin("completions_demo").expect("Failed to get command for completions_demo example");
    cmd.arg("completions").arg("bash");
    
    let assert = cmd.assert().success();
    let stdout = String::from_utf8(assert.get_output().stdout.clone()).expect("Output is not valid UTF-8");
    
    assert!(stdout.contains("# completions_demo completion script for bash"), "Output missing header for bash");
...
fn test_completions_e2e_zsh() {
    ...
    assert!(stdout.contains("# completions_demo completion script for zsh"), "Output missing header for zsh");
...
fn test_completions_e2e_fish() {
    ...
    assert!(stdout.contains("# completions_demo completion script for fish"), "Output missing header for fish");
    assert!(stdout.contains("complete -c completions_demo"), "Output missing complete command for fish");
    assert!(stdout.contains("services status"), "Output missing services status subcommands");
}
...
fn test_completions_e2e_powershell() {
    ...
    assert!(stdout.contains("Register-ArgumentCompleter"), "Output missing Register-ArgumentCompleter for powershell");
