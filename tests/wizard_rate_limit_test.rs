use clap_noun_verb::{CliBuilder, VerbArgs, NounVerbError, VerbCommand, NounCommand};
...
impl VerbCommand for SimpleVerb {
    ...
    fn about(&self) -> &'static str { self.about }
...
impl NounCommand for RateLimitNoun {
    fn name(&self) -> &'static str { "wizard" }
    fn about(&self) -> &'static str { "Wizard" }
...
fn test_wizard_handler_rate_limiting() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let noun1 = RateLimitNoun {
        handler: handler1,
    };

    let cli = CliBuilder::new()
        .name("wizard-rate-limit")
        .noun(noun1);

    let cmd = cli.build_command();

    // First call: should succeed
    let res = cmd.try_get_matches_from(vec!["wizard-rate-limit", "wizard", "generate"])
        .map_err(|e| e.to_string())?;

    // Execute the handler manually to simulate rate-limiting trigger
    let sub1 = res.subcommand_matches("wizard")
        .ok_or("missing wizard subcommand")?.clone();
    let sub2 = sub1.subcommand_matches("generate")
        .ok_or("missing generate subcommand")?.clone();
    ...
    assert!(res2.is_err());
    assert_eq!(res2.err().ok_or("expected err")?.to_string(), "Command execution failed: Rate limit exceeded");
