...
fn test_record_capability_granted() {
    ...
    assert_eq!(event_id.unwrap(), EventId::new(0));
}
...
fn test_ledger_checkpoint() {
    ...
    assert_eq!(ledger.event_count().unwrap(), 6);
}
...
fn test_ledger_query_by_time_range() {
    ...
    let all_events = ledger.query().time_range(start, end).execute().unwrap();

    let middle_events = ledger.query().time_range(middle, end).execute().unwrap();
...
fn test_ledger_query_by_agent() {
    ...
    let alice_events = ledger.query().agent("alice".to_string()).execute().unwrap();
    let bob_events = ledger.query().agent("bob".to_string()).execute().unwrap();
...
fn test_ledger_query_by_correlation_id() {
    ...
    let batch1_events = ledger.query().correlation_id("batch-1".to_string()).execute().unwrap();
...
fn test_ledger_query_count() {
    ...
    let count = ledger.query().agent("alice".to_string()).count().unwrap();
...
fn test_governance_event_types() {
    ...
    assert_eq!(ledger.event_count().unwrap(), 3);
}
...
fn test_ledger_metadata_preservation() {
    ...
    let events = ledger.query().correlation_id(correlation.to_string()).execute().unwrap();
