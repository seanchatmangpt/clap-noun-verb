...
#[verb("set", "argmeta")]
fn set_config(
    #[arg(short = 'p', default_value = "8080", value_name = "PORT")] port: u16,
    #[arg(short = 'o', default_value = "stdout", value_name = "DEST")] output: String,
) -> Result<ConfigResult> {
...
fn parameter_attributes_reach_the_clap_command() -> Result<()> {
    ...
    let noun = command
        ...
        .ok_or_else(|| {
            clap_noun_verb::error::NounVerbError::execution_error("argmeta noun missing")
        })?;
    let verb = noun
        .get_subcommands()
    ...
    let output =
        verb.get_arguments().find(|argument| argument.get_id().as_str() == "output").ok_or_else(
            || clap_noun_verb::error::NounVerbError::execution_error("output arg missing"),
        )?;
