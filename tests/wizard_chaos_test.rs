...
fn test_wizard_chaos_argument_sequences() {
    ...
    let res = cmd.clone().try_get_matches_from(vec!["wizard-chaos", "wizard", "generate", "--verbose"]);
    assert!(res.is_err());

    // Chaotic input 2: unexpected extra positional arguments
    let res = cmd.clone().try_get_matches_from(vec![
        "wizard-chaos", "wizard", "generate", "--target", "foo", "extra_pos1", "extra_pos2"
    ]);
    assert!(res.is_err());

    // Chaotic input 3: repeating options
    let res = cmd.clone().try_get_matches_from(vec![
        "wizard-chaos", "wizard", "generate", "--target", "foo", "--target", "bar"
    ]);
