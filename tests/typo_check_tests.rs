...
impl NounCommand for TestNoun {
    ...
    fn verbs(&self) -> Vec<Box<dyn VerbCommand>> {
        if self.name == "service" {
            vec![Box::new(TestVerb { name: "status" }), Box::new(TestVerb { name: "restart" })]
        } else {
            vec![]
        }
    }
...
fn test_typo_check_noun_suggestions() {
    ...
    let mock_cmd = Command::new("cli").subcommand(Command::new("servise"));
...
fn test_typo_check_verb_suggestions() {
    let registry = CommandRegistry::new().register_noun(TestNoun { name: "service" });

    // Build a mock command structure that allows parsing the typo "statos"
    let mock_cmd =
        Command::new("cli").subcommand(Command::new("service").subcommand(Command::new("statos")));
