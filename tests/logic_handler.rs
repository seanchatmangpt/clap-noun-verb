...
#[test]
fn test_handler_output_from_data() {
    let output = HandlerOutput::from_data("All services running").expect("Failed to create output");

    let json_str = output.to_json().expect("Failed to serialize");
    assert!(json_str.contains("All services running"));
}

#[test]
fn test_handler_output_with_serializable() {
    use serde::Serialize;
    ...
    let output = HandlerOutput::from_data(Status {
...
fn test_handler_output_with_message() {
    let output = HandlerOutput::from_data("Test")
        .expect("Failed to create output")
