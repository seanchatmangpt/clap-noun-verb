...
3. **Incomplete error handling** - Error paths not fully handled (e.g., `unwrap_or` instead of `Result`)
...
// 1. Function uses unwrap_or - incomplete error handling
// 2. Type only used in tests - incomplete adoption
...
### Error Handling
- Function uses unwrap_or (should return Result)
- Some error paths not fully handled

### Type Safety
- usize for indices (should use newtype)
...
### Adoption
- Types only used in tests (not production)
```
...
1. Fix error handling to return Result - Prevents silent failures, maintains error handling consistency
2. Add compile-fail test - Verifies claims, maintains test quality
3. Use Rust in Rust project - Maintains consistency, prevents language mixing
4. Add newtype for indices - Prevents index errors, maintains type safety

### High Impact, Medium Value (Good Work - Plan)
5. Add const generic validation - Compile-time validation, maintains quality
6. Add error path tests - Complete test coverage, maintains quality standards

### Foundation Work (High Value, Lower Impact)
7. Migrate production code to use validated types - Incremental adoption, maintains consistency
```
...
```rust
// Example: Finish error handling capability
...
4. Additional error path tests - In progress
5. Integration tests - In progress

### Future (Lower Priority)
6. Production code migration - Plan for later
7. Comprehensive test coverage - Incremental
```
...
# Step 3: Finish Capabilities
# - Fixed error handling to return Result
# - Added compile-fail test
# - Added newtype for indices
...
# - Next: Complete error path tests
```
...
- **[Verify Tests](./verify-tests.md)** - Use to validate completed capabilities
