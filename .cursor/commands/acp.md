...
**Action**: Run validation to identify all issues before proceeding.
...
**What this does**:
- Formats code (`cargo make format-check`)
- Lints code (`cargo make lint`)
- Runs tests (`cargo make test-timeout`)

**Expected Result**: All checks pass (exit code 0)

**CRITICAL**: Validation checks MUST pass before committing. CI/CD will block commits
with compilation errors. Always run `cargo make verify` before committing.
...
**Validation**: `cargo make format` completes without errors
...
**Action**: Re-run validation to ensure all issues are fixed.
...
7. **Never use --no-verify** - Git hooks MUST run to enforce quality gates
...
cargo make verify          # Step 1: Validate
# Fix issues if needed     # Step 2: Fix
cargo make verify          # Step 3: Re-validate
