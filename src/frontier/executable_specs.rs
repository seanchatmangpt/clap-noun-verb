...
impl ExecutableSpec {
        ...
        F: Fn(&HashMap<&str, usize>) -> bool,
    {
        ...
        let params = HashMap::from([("total_nodes", 10), ("byzantine_nodes", 3)]);

        if property(&params) {
            Ok(true)
        } else {
            Err(SpecError::PropertyFailed(format!("Property {} failed validation", self.name)))
        }
    }
...
impl SpecificationSuite {
    ...
    pub fn get_spec(&self, name: &str) -> Result<&ExecutableSpec> {
        self.specs.get(name).ok_or_else(|| SpecError::NotFound(name.to_string()))
    }
...
mod tests {
    ...
    fn test_spec_creation() {
        let spec = ExecutableSpec::new("Test Spec", "A test specification");
    ...
    fn test_spec_validation() {
        let spec =
            ExecutableSpec::new("Byzantine Consensus", "System tolerates up to f Byzantine nodes");
