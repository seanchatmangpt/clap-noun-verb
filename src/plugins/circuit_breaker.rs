...
impl CircuitBreakerPlugin {
    pub fn new() -> Self { Self { state: Arc::new(Mutex::new(CircuitState::Closed)), failures: Arc::new(Mutex::new(0)), loaded: false } }
    pub fn call<F>(&self, op: F) -> crate::Result<String> where F: FnOnce() -> crate::Result<String> { op() }
}

impl Default for CircuitBreakerPlugin { fn default() -> Self { Self::new() } }
