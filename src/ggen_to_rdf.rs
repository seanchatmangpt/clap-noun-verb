...
fn parse_function_signature(
    ...
    // Extract return type
    let return_type = extract_return_type(&normalized_sig)
...
pub fn verb_definitions_to_ntriples(verbs: &[RdfVerbDefinition]) -> String {
    ...
    ntriples.push_str(&std::time::SystemTime::now()
...
pub fn verb_definitions_to_sparql_insert(
    verbs: &[RdfVerbDefinition],
) -> String {
    ...
    for verb in verbs {
        sparql.push_str(&format!("  ex:{} a cnv:Verb ;\n", verb.verb_uri.split('/').last().unwrap_or("Verb")));
        ...
        if let Some(noun_uri) = &verb.noun_uri {
            sparql.push_str(&format!(
                ...
                noun_uri.split('/').last().unwrap_or("Noun")
            ));
        }
        ...
        for arg in &verb.arguments {
            sparql.push_str(&format!(
                "    cnv:hasArguments ex:{} ;\n",
                arg.arg_uri.split('/').last().unwrap_or("Arg")
            ));
        }
        ...
        // Arguments
        for arg in &verb.arguments {
            sparql.push_str(&format!("  ex:{} a cnv:Argument ;\n", arg.arg_uri.split('/').last().unwrap_or("Arg")));
