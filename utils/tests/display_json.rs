...
use clap_noun_verb_utils::display_json::{
    extract_command_schema, arg_matches_to_json, PrintJson
};
...
fn test_command_schema_serialization() -> Result<(), String> {
    ...
    // Find verbose arg
    let verbose_arg = schema.arguments.iter().find(|a| a.name == "verbose")
    ...
    let start_sub = schema.subcommands.iter().find(|s| s.name == "start")
...
fn test_arg_matches_to_json() -> Result<(), String> {
    let cmd = create_test_command();
    let matches = cmd.try_get_matches_from(vec!["test-cli", "--port", "8080", "--verbose", "--tag", "tag1", "--tag", "tag2"])
        .map_err(|e| format!("Failed to parse matches: {}", e))?;
        
    let json_val = arg_matches_to_json(&matches);
    
    let port_val = json_val.get("port")
        .ok_or_else(|| "port not found in JSON matches".to_string())?;
    assert_eq!(port_val, &Value::Number(8080.into()));
    
    let verbose_val = json_val.get("verbose")
        .ok_or_else(|| "verbose not found in JSON matches".to_string())?;
    assert_eq!(verbose_val, &Value::Bool(true));

    let tags_val = json_val.get("tags")
        .ok_or_else(|| "tags not found in JSON matches".to_string())?;
    assert_eq!(tags_val, &Value::Array(vec![Value::String("tag1".to_string()), Value::String("tag2".to_string())]));
