...
use clap_noun_verb_utils::display_json::{
    extract_command_schema, arg_matches_to_json,
};
...
fn test_parse_duration_adversarial() {
    ...
    assert!(parse_duration("18446744073709551615m").is_err(), "Expected Err on parse_duration overflow for u64::MAX minutes");
}
...
fn test_arg_matches_to_json_custom_type() {
    ...
    let matches = cmd.clone().try_get_matches_from(vec!["custom-app", "--custom", "hello-world", "--flag"]).unwrap();
    ...
    let cmd_default = Command::new("custom-app")
        .arg(
            Arg::new("custom")
                .long("custom")
                .action(ArgAction::Set)
