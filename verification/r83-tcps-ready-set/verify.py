...
EXPECTED_CONSUMER = "seanchatmangpt/clap-noun-verb"

def classify(s):
    if s.get("consumer_repo") != EXPECTED_CONSUMER: return "REFUSED[FOREIGN_CONSUMER]"
...
def main():
    ...
    for case in CASES:
        candidate=copy.deepcopy(SUBJECT); candidate.update(case.get("set", {}))
        actual=classify(candidate); print(case["id"] + "=" + actual)
    ...
    print("R83_CONSUMER=" + standing); print("SUBJECT_DIGEST=" + digest); print("CASE_COUNT=" + str(len(CASES)))
